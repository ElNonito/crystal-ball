...
class BaseTextCategorizationDataset:
    ...
    def get_num_labels(self):
        ...
        """
        return len(set(self._get_label_list()))
    ...
    def get_label_to_index_map(self):
        ...
        inv_map = {label : index for index, label in index_to_label_map.items()}
        return inv_map
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        assert list(dataset) == ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title']
        ...
        def filter_tag_position(position):
            def filter_function(df):
                ...
                """
                dataset = df.loc[df['tag_position'] == position]
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                counts = df['tag_name'].value_counts()
                cleaned = df[df['tag_name'].isin(counts[counts >= x].index)]
                return cleaned
            return filter_function
